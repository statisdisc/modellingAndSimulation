class finiteVolumeFunctions:
    ...
    def div(self, field, u, scheme):
        divField = mesh.volScalarField.copy()
        ...
        fluxx = fieldFacex*dot(uFacex, mesh.xSf)/mesh.cellVolume
        if not mesh.xPeriodic:
            fluxx[:,0] *= 0.
        
        fluxz = fieldFacez*dot(uFacez, mesh.zSf)/mesh.cellVolume
        if not mesh.zPeriodic:
            fluxz[-1,:] *= 0.
